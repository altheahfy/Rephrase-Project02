...
with open("slottext.json", "r", encoding="utf-8") as f:
    slottext_rules = json.load(f)

def infer_slot_text(phrase, slot_name):
    phrase_lc = phrase.lower()
    matches = []

    for rule in slottext_rules["rules"]:
        condition = rule["condition"]
        guidance = rule["guidance"]
        m = re.search(condition, phrase_lc, re.IGNORECASE)
        if m:
            # had の過去完了 guidance は had to の場合は付与しない
            if guidance == "過去完了" and phrase_lc.startswith("had to"):
                continue
...
for row in data_rows:
    ...
    question_type = safe_strip(row_dict.get("QuestionType", ""))
