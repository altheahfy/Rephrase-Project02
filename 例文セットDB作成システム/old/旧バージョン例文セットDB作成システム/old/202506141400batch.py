...
# ✅ 差分追加: subslot_structure への記載処理
existing_subslots = set((row[0].value.strip(), row[1].value.strip()) for row in substructure_sheet.iter_rows(min_row=2))
...
for row in mapping_sheet.iter_rows(min_row=2, values_only=True):
    ...
    if parent_phrase.strip() not in current_parent_phrases:
        continue  # 他例文のデータは無視
    key = (sub_phrase.strip(), sub_slot.strip())
    if key not in existing_subslots:
        sub_text = infer_slot_text(sub_phrase.strip())
        substructure_sheet.append([sub_phrase.strip(), sub_slot.strip(), sub_text])
        existing_subslots.add(key)
