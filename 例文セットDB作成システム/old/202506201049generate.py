...
def main():
    ...
    slot_order = ["M1", "S", "Aux", "V", "O1", "O2", "C1", "C2", "M2", "M3"]
    ...
    for eid, group in df_out[df_out["SubslotID"] == ""].groupby("例文ID"):
        ...
        for i, idx in enumerate(ordered["index"]):
            ...
            for j in df_out[(df_out["例文ID"] == eid) & (df_out["Slot"] == sid) & (df_out["SubslotID"] != "")].index:
                df_out.at[j, "Slot_display_order"] = order_val
            # サブスロット行に値をコピー
            mask = (df_out["例文ID"] == eid) & (df_out["Slot"] == df_out.at[idx, "Slot"]) & (df_out["SubslotID"] != "")
            df_out.loc[mask, "Slot_display_order"] = i + 1
