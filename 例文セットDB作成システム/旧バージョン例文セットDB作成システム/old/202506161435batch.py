...
# ✅ structure_sheet 出力（slot_display_order を含む）
header = [cell.value for cell in structure_sheet[1]]
if 'slot_display_order' not in header:
    structure_sheet.cell(row=1, column=len(header) + 1).value = 'slot_display_order'
    order_col_idx = len(header) + 1
else:
    order_col_idx = header.index('slot_display_order') + 1

for row_idx, slot in enumerate(data["slot_data"], start=2):
    ...
    structure_sheet.cell(row=row_idx, column=order_col_idx).value = slot.get("slot_display_order", "")
