...
def infer_slot_text(phrase, slot_name=None, subslots=None, idx=None):
    ...
    if subslots is not None and idx is not None:
        if idx > 1:
            ...
            next_v_slot = None
            if idx < len(subslots) and subslots[idx].get("slot", "") in ["sub-v", "V"]:
                next_v_slot = subslots[idx]
            elif idx + 1 < len(subslots) and subslots[idx + 1].get("slot", "") in ["sub-v", "V"]:
                next_v_slot = subslots[idx + 1]

            if next_v_slot and slot_name in ["V", "sub-v"]:
