# 統合Stanza-Rephraseマッパー マイグレーション計画

**最終更新**: 2025年8月15日  
**Phase 2完了**: 関係節・受動態統合処理（98.1%成功率達成）  
**設計原則**: 並列処理・責任分離・競合回避準拠

## 🎯 エンジン統合設計原則 ⭐⭐⭐ 必須遵守

### 並列処理基本方針
1. **独立解析**: 各エンジンが全文を独立して解析
2. **責任分離**: エンジン間で処理対象を明確に分割  
3. **競合回避**: 空値での有効値上書きを防止
4. **重複排除**: エンジン内での他エンジン呼び出し禁止

**詳細仕様**: [ENGINE_INTEGRATION_DESIGN_PRINCIPLES.md](../../../設計仕様書/ENGINE_INTEGRATION_DESIGN_PRINCIPLES.md)
...
- [ ] **article_determiner_engine.py** (新規作成 - Phase 3)
  ...
  - 設計原則: ✅**既存値拡張方式**（"car" → "The car"）、空値での上書き禁止
  - 重要度: ⭐⭐ **名詞句抽出完全性・必須**
