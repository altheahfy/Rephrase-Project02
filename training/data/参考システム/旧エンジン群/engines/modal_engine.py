...
def main():
    ...
    print("噫 Modal Engine Test Results")
    print("=" * 80)
    ...
    for i, sentence in enumerate(test_sentences, 1):
        ...
        if engine.is_applicable(sentence):
            ...
            print(f"  笨・Applicable (Pattern: {pattern_type})")
            print(f"  投 Confidence: {confidence:.2f}")
            print(f"  肌 Modal Info: {modal_info['modal_found']} ({modal_info['modal_function']})")
            print(f"  統 Slots: {slots}")
        else:
            print("  笶・Not applicable")
