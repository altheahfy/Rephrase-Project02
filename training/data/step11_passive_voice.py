"""
Step 11統合版 - Passive Voice (受動態) 対応
Step 10をベースに受動態パターンを安全に追加
受動態：be動詞→Aux、by句→M2の設計思想に基づく
"""

import re
from datetime import datetime

class Step11RuleEngine:
    """Step 11ルールエンジン（Passive Voice対応版）"""
    ...
    def __init__(self):
        ...
        self.modal_rules = []             # モーダル助動詞
        self.passive_rules = []           # 受動態 (Step 11新規)
        self.init_all_rules()
    ...
    def init_all_rules(self):
        ...
        # Step 11の受動態ルール（新規追加）
        self.passive_rules = [
            self.rule_passive_be_past_participle,  # be + 過去分詞
            self.rule_passive_by_agent,             # by + 動作主
        ]
        ...
        for i, word in enumerate(words):
            if word.lower() == 'the' and i < len(words) - 1:
                ...
                common_nouns = ['weather', 'music', 'food', 'tree', 'book', 'car', 'house', 
                              ...
                              'window', 'door', 'problem', 'question', 'answer', 'story', 'letter',
                              'homework', 'work', 'job', 'project', 'report', 'test', 'exam']
    ...
    def rule_v_be_exist_loc(self, words):
        ...
        for i, word in enumerate(words):
            if word.lower() in be_forms:
                ...
                if i < len(words) - 1:
                    next_word = words[i + 1]
                    # 進行形（ing）でも受動態（過去分詞）でもない場合
                    if (not next_word.lower().endswith('ing') and 
                        not self.is_past_participle(next_word.lower())):
                        results.append({
                            'position': i,
                            'slot': 'V',
                            'value': word,
    ...
    def is_past_participle(self, word):
        ...
        irregular_past_participles = [
            'done', 'gone', 'seen', 'been', 'taken', 'made', 'said', 'come', 'written',
            'broken', 'spoken', 'chosen', 'driven', 'eaten', 'fallen', 'given', 'hidden',
            'known', 'ridden', 'risen', 'shaken', 'shown', 'stolen', 'thrown', 'worn',
            'built', 'bought', 'brought', 'caught', 'fought', 'sought', 'taught', 'thought',
            'found', 'heard', 'held', 'left', 'lost', 'met', 'paid', 'sent', 'sold', 'told'
        ]
        return word in irregular_past_participles
    
    def rule_passive_be_past_participle(self, words):
        """受動態：be動詞 + 過去分詞パターン"""
        results = []
        ...
        for i, word in enumerate(words):
            if word.lower() in be_forms:
                ...
                if i < len(words) - 1:
                    next_word = words[i + 1].lower()
                    if self.is_past_participle(next_word):
                        # be動詞を受動態助動詞として扱う（Rephraseの設計思想）
                        results.append({
                            'position': i,
                            'slot': 'Aux',
                            'value': word,
                            'type': 'word',
                            'rule_id': 'passive-be-aux',
                            'priority': 11,  # 進行形より高優先度
                            'note': f'受動態助動詞（後続：{words[i + 1]}）',
                            'pattern': 'passive'
                        })
                        ...
                        results.append({
                            ...
                            'value': words[i + 1],
                            'type': 'word',
                            'rule_id': 'passive-past-participle',
                            'priority': 11,
                            'note': '過去分詞動詞（受動態）',
                            'pattern': 'passive'
                        })
    ...
    def rule_passive_by_agent(self, words):
        """受動態：by + 動作主のM2処理"""
        results = []
        
        for i, word in enumerate(words):
            if word.lower() == 'by' and i < len(words) - 1:
                # by + 動作主のパターン（受動態での動作主）
                agent = words[i + 1]
                
                # 代名詞や一般的な名詞をチェック
                if (agent.lower() in ['me', 'you', 'him', 'her', 'us', 'them', 'it'] or
                    agent.lower() in ['john', 'mary', 'tom', 'alice', 'bob'] or
                    agent.lower() in ['teacher', 'student', 'doctor', 'police', 'government']):
                ...
                # 冠詞付きの場合（by the teacher等）
                elif agent.lower() in ['the', 'a', 'an'] and i < len(words) - 2:
                    agent_phrase = f'{word} {agent} {words[i + 2]}'
                    results.append({
                        'position': i,
                        'slot': 'M2',
                        'value': agent_phrase,
                        'type': 'phrase',
                        'rule_id': 'passive-by-agent-article',
                        'priority': 8,
                        'span': 3,
                        'note': '動作主（by + 冠詞 + 名詞）',
                        'pattern': 'passive'
    ...
    def resolve_conflicts(self, candidates):
        """競合解決（優先度ベース + ポジション重複解決 + スロット重複解決 + モーダル+受動態対応）"""
        if not candidates:
            return {}
        ...
        for pos, group in position_groups.items():
            ...
            if slot not in final_slots:
                ...
                # 既にスロットにエントリがある場合
                # Auxスロットでは複数の助動詞を連結する場合がある（modal + be）
                if slot == 'Aux':
                    existing = final_slots[slot][0]
                    new_pattern = best_candidate.get('pattern', '')
                    existing_pattern = existing.get('pattern', '')
                    
                    # モーダル + 受動態の組み合わせを統合
                    if ((new_pattern == 'modal' and existing_pattern == 'passive') or
                        (new_pattern == 'passive' and existing_pattern == 'modal')):
                        # モーダル助動詞 + 受動態助動詞を結合
                        if new_pattern == 'modal':
                            modal_value = best_candidate['value']
                            passive_value = existing['value']
                            modal_note = best_candidate.get('note', '')
                        else:
                            modal_value = existing['value']
                            passive_value = best_candidate['value']
                            modal_note = existing.get('note', '')
                        
                        # 統合された値を作成
                        combined_value = f"{modal_value} {passive_value}"
                        combined_note = f"{modal_note} + 受動態助動詞"
                        
                        final_slots[slot] = [{
                            'value': combined_value,
                            'type': 'モーダル+受動態助動詞',
                            'rule_id': f"{existing['rule_id']}+{best_candidate['rule_id']}",
                            'position': min(existing['position'], pos),
                            'note': combined_note,
                            'pattern': 'modal+passive'
                        }]
                    # モーダル + 完了形の組み合わせを統合
                    elif ((new_pattern == 'modal' and existing_pattern == 'perfect') or
                          (new_pattern == 'perfect' and existing_pattern == 'modal')):
                        # モーダル助動詞 + 完了形助動詞を結合
                        if new_pattern == 'modal':
                            modal_value = best_candidate['value']
                            perfect_value = existing['value']
                            modal_note = best_candidate.get('note', '')
                        else:
                            modal_value = existing['value']
                            perfect_value = best_candidate['value']
                            modal_note = existing.get('note', '')
                        
                        # 統合された値を作成
                        combined_value = f"{modal_value} {perfect_value}"
                        combined_note = f"{modal_note} + 完了形助動詞"
...
def main():
    """Step 11テスト実行"""
    engine = Step11RuleEngine()
    
    print("=== Step 11 Passive Voice統合テスト ===\n")
    ...
    test_sentences = [
        ...
        "She must have finished the work",
        "They might have been there",
        
        # 複合テスト：受動態 + 修飾語
        "The book is written very well by John",
        "The window was broken yesterday by the storm",
