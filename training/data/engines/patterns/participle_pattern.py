...
from training.data.engines.patterns.base_pattern import GrammarPattern
from training.data.engines.universal_slot_position_manager import UniversalSlotPositionManager
...
class ParticiplePattern(GrammarPattern):
    ...
    def detect(self, analysis_doc, sentence):
        ...
        for i, token in enumerate(tokens):
            ...
            # 複合分詞 (being + 過去分詞)
                ...
                if next_token.upos == 'VERB' and next_token.feats and 'VerbForm=Part' in str(next_token.feats):
                    participle_detected = True
    ...
    def correct(self, analysis_doc, sentence):
        ...
        # 分詞構文の特殊処理
        enhanced_result = self._enhance_participle_slots(analysis_doc, unified_result, sentence)
