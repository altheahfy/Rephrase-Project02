...
### 🎯 **Mスロット割り当ての基本原理：「文の中央からの距離」**

**【最重要概念】**
- **文の中央（動詞周辺）からの論理的距離**で判定
- **M2を優先使用**（動詞に近い修飾語）
- **常にMスロットに余裕を残す**（後続拡張の可能性を考慮）

**【具体的判定基準】**
- **M2**: 文の中央（動詞周辺）に**近い**修飾語 → **優先使用**
- **M1**: 文の中央から**遠い**（文頭寄り）修飾語
- **M3**: 文の中央から**遠い**（文尾寄り）修飾語

**【重要な考え方】**
```
❌ 間違った判断: 「文尾だからM3」
✅ 正しい判断: 「動詞からの距離が近いからM2」
```

**【拡張可能性の考慮】**
- 文に後続部分が続く可能性（`because this house is huge.` など）
- 無理にM3を使わず、M2が適切なら**M2を使用**
- **余裕のあるMスロット配置**で将来の拡張に対応

**配置例1：基本パターン**
```
"Yesterday, I carefully finished my work early."
M1: "Yesterday"    ← 文頭位置（中央から遠い）
M2: "carefully"    ← 動詞周辺位置（中央に近い）
M3: "early"        ← 文尾位置（中央から遠い）
```
...
**【追加配置原則】**
- 複数の修飾語がある場合、**動詞に最も近いもの**を**M2**
- 文頭・文尾の修飾語は**M1, M3**
- **拡張可能性**を常に意識した余裕のある配置
