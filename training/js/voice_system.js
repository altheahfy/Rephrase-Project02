...
class VoiceSystem {
    ...
    setupEventListeners() {
        ...
        console.log(`🔍 デバイス検出: Android=${isAndroid}, iOS=${isIOS}`);
        
        // 録音ボタン（デバイス別機能切り替え）
        const recordBtn = document.getElementById('voice-record-btn');
        if (recordBtn) {
            if (isIOS) {
                // iPhone: 録音+音声認識同時処理（既存機能そのまま）
                recordBtn.addEventListener('click', () => this.toggleRecording());
                recordBtn.title = '🎤 録音+音声認識（同時処理）';
                console.log('✅ iPhone: 録音ボタンに同時処理機能を設定');
            } else if (isAndroid) {
                // Android: 録音のみ（音声認識は分析ボタンで別処理）
                recordBtn.addEventListener('click', () => this.toggleRecordingOnly());
                recordBtn.title = '🎤 録音のみ（Android制限対応）';
                console.log('✅ Android: 録音ボタンに録音のみ機能を設定');
                
                // Android用: 分析ボタンを表示して音声認識機能を割り当て
                this.setupAnalyzeButtonForAndroid();
            } else {
                // その他デバイス: デフォルトで同時処理
                recordBtn.addEventListener('click', () => this.toggleRecording());
                recordBtn.title = '🎤 録音+音声認識';
                console.log('✅ その他デバイス: 録音ボタンにデフォルト機能を設定');
            }
        }
...
/**
 * 🤖 Android専用: 録音のみ機能（音声認識なし）
 */
VoiceSystem.prototype.toggleRecordingOnly = async function() {
    console.log('🤖 Android: 録音のみモード実行');
    
    if (this.isRecording) {
        this.stopRecordingOnly();
    } else {
        await this.startRecordingOnly();
    }
};

/**
 * 🤖 Android専用: 録音のみ開始（音声認識なし）
 */
VoiceSystem.prototype.startRecordingOnly = async function() {
 ...
 * 🤖 Android専用: 録音のみ停止
 */
VoiceSystem.prototype.stopRecordingOnly = function() {
...
/**
 * 🤖 Android専用: 分析ボタンセットアップ
 */
VoiceSystem.prototype.setupAnalyzeButtonForAndroid = function() {
    console.log('🤖 Android: 分析ボタンをセットアップ');
    
    const analyzeBtn = document.getElementById('voice-analyze-btn');
    if (analyzeBtn) {
        // 分析ボタンを表示
        analyzeBtn.style.display = 'inline-block';
        analyzeBtn.title = '🔍 音声認識分析（Android専用）';
        
        // 音声認識機能を追加
        analyzeBtn.addEventListener('click', () => this.startVoiceRecognitionOnly());
        
        console.log('✅ Android: 分析ボタンに音声認識機能を設定');
    } else {
        console.warn('⚠️ 分析ボタンが見つかりません');
    }
};
 ...
 * 🤖 Android専用: 音声認識のみ実行
 */
VoiceSystem.prototype.startVoiceRecognitionOnly = function() {
    console.log('🤖 Android: 音声認識のみ開始');
    
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
        this.updateStatus('❌ このブラウザは音声認識をサポートしていません', 'error');
        return;
    }
    ...
    this.recordingRecognition.interimResults = true;
    this.recordingRecognition.maxAlternatives = 1;
    this.recordingRecognition.lang = 'en-US';
    
    // タイムアウト設定
    const timeoutDuration = 15000; // 15秒
    this.recognitionTimeoutId = setTimeout(() => {
        this.recordingRecognition.stop();
        console.log('⏰ Android音声認識タイムアウト');
    }, timeoutDuration);
    
    // イベントハンドラー
    this.recordingRecognition.onstart = () => {
        this.isRecognitionActive = true;
        console.log('✅ Android音声認識開始');
        this.updateStatus('🤖 音声認識中...（Android分析モード）', 'recognizing');
    };
    ...
    this.recordingRecognition.onresult = (event) => {
        for (let i = event.resultIndex; i < event.results.length; i++) {
            ...
            if (result.isFinal) {
                ...
                console.log('✅ Android認識結果（確定）:', transcript);
            } else {
    ...
    this.recordingRecognition.onend = () => {
        ...
        if (this.recognizedText.trim()) {
            ...
            console.log('✅ Android音声認識完了:', this.recognizedText);
        } else {
            this.updateStatus('⚠️ 音声を認識できませんでした', 'warning');
        }
    };
    ...
    this.recordingRecognition.onerror = (event) => {
        ...
        console.error('❌ Android音声認識エラー:', event.error);
        this.updateStatus('❌ 音声認識エラーが発生しました', 'error');
    };
    ...
    try {
        ...
        console.error('❌ Android音声認識開始失敗:', error);
        this.updateStatus('❌ 音声認識開始に失敗しました', 'error');
    }
};