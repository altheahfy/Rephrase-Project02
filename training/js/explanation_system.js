    ...
    try {
      ...
      if (window.lastSelectedSlots && window.lastSelectedSlots.length > 0) {
        ...
        if (vSlot && vSlot.V_group_key) {
          console.log('✅ window.lastSelectedSlotsのVスロットから取得:', vSlot.V_group_key);
          return vSlot.V_group_key;
        }
        // 任意のスロットからV_group_keyを取得（すべて同じV_group_keyのはず）
      ...
      // フォールバック1: window.currentRandomizedState
      if (window.currentRandomizedState && window.currentRandomizedState.vGroupKey) {
        console.log('⚠️ フォールバック: window.currentRandomizedState.vGroupKeyから取得:', window.currentRandomizedState.vGroupKey);
        return window.currentRandomizedState.vGroupKey;
      }
      
      // フォールバック2: メインエリアの全てのスロット要素を検索
