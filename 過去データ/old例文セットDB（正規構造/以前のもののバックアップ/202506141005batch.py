...
with open("slottext_people.json", "r", encoding="utf-8") as f:
    people_rules = json.load(f)["rules"]

# ✅ 補助テキスト自動補完（人名対応＋ログ対象）
def infer_slot_text(phrase):
    phrase_lc = phrase.lower()
    for rule in people_rules:
        condition = rule["condition"]
        guidance = rule["guidance"]
        if re.search(condition, phrase):
            return f"{guidance}{phrase}"
...
existing_pairs = set(
    ((row[1].value or "").strip(), (row[2].value or "").strip())
...
for slot in data["slot_data"]:
    if slot["type"] == "word":
        ...
        slottext_log.append(f"Slot: {slot['slot']} Phrase: {slot['phrase']} → SlotText: {slot_text}")
