...
function applyMultipleImagesToSlot(slotId, phraseText, forceRefresh = false) {
  ...
  if (!imageContainer) {
    ...
    // 🎯 サブスロット画像配置責任：universal_image_system.jsがGrid行2を完全管理
    // サブスロットの場合、画像はGrid行2に固定配置（CSS側で定義済み）
    if (isSubslot) {
      console.log(`🎯 サブスロット画像配置: ${slotId} → Grid行2に固定配置`);
      // CSS側で.subslot-container .multi-image-containerが既にgrid-row: 2に設定済み
      // JavaScriptでの行指定は不要（CSS責任）
    }
