...
function getEnglishTextFromSlotPool(subslotId, parentSlotId) {
  // 🔍 デバッグ：M1、S、M2、O1、O2サブスロット問題調査のため一時的にDOMから直接取得
  if (subslotId.includes('slot-m1-sub-') || subslotId.includes('slot-s-sub-') || subslotId.includes('slot-m2-sub-') || subslotId.includes('slot-o1-sub-') || subslotId.includes('slot-o2-sub-')) {
    console.log(`🔍 M1、S、M2、O1、またはO2サブスロットのため、DOM直接取得でテスト: ${subslotId}`);
...
function updateSubslotImages(parentSlotId) {
  ...
  // 🎯 水平展開段階：C1、M1、S、M2、O1、およびO2スロットに対応
  if (parentSlotId !== 'c1' && parentSlotId !== 'm1' && parentSlotId !== 's' && parentSlotId !== 'm2' && parentSlotId !== 'o1' && parentSlotId !== 'o2') {
    console.log(`⏭️ 水平展開段階のため ${parentSlotId} はスキップします（C1、M1、S、M2、O1、O2スロットのみ対象）`);
    return;
  }
