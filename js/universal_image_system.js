...
// 🔧 緊急デバッグ：実際の表示状態を確認
function emergencySubslotImageDebug() {
  console.log('🚨 ==========================================');
  console.log('🚨 緊急デバッグ：サブスロット画像表示状態調査');
  console.log('🚨 ==========================================');
  
  const allSubslots = document.querySelectorAll('[id*="-sub-"]');
  console.log('📍 発見されたサブスロット数:', allSubslots.length);
  
  allSubslots.forEach((subslot, index) => {
    const imgElement = subslot.querySelector('.slot-image');
    console.log(`\n🔍 サブスロット ${index + 1}: ${subslot.id}`);
    ...
    if (imgElement) {
      ...
      console.log('  🔍 width:', window.getComputedStyle(imgElement).width);
      console.log('  🔍 height:', window.getComputedStyle(imgElement).height);
      console.log('  🔍 position:', window.getComputedStyle(imgElement).position);
      console.log('  🔍 z-index:', window.getComputedStyle(imgElement).zIndex);
      console.log('  🔍 data-meta-tag:', imgElement.getAttribute('data-meta-tag'));
      console.log('  🔍 classes:', imgElement.className);
      ...
      console.log('  📺 画面内表示:', isInViewport);
      ...
      while (parent && parentLevel <= 3) {
        ...
        console.log(`  👆 親${parentLevel} (${parent.tagName}${parent.id ? '#' + parent.id : ''}):`, {
  ...
  console.log('\n🚨 ==========================================');
  console.log('🚨 緊急デバッグ完了');
  console.log('🚨 ==========================================');
}

// グローバルに公開
window.emergencySubslotImageDebug = emergencySubslotImageDebug;

// 🔧 MutationObserver で画像が消される瞬間を監視
function watchImageDisappearance() {
  console.log('👁️ 画像消失監視を開始...');
  
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      if (mutation.type === 'attributes' && mutation.target.classList.contains('slot-image')) {
        const img = mutation.target;
        if (img.closest('[id*="-sub-"]')) {
          console.log('🚨 サブスロット画像の属性変更を検出:', {
            subslotId: img.closest('[id*="-sub-"]').id,
            attributeName: mutation.attributeName,
            oldValue: mutation.oldValue,
            newValue: img.getAttribute(mutation.attributeName),
      ...
      if (mutation.type === 'childList') {
        mutation.removedNodes.forEach((node) => {
          if (node.nodeType === 1 && node.classList && node.classList.contains('slot-image')) {
            console.log('🚨 サブスロット画像要素が削除されました:', node);
          }
        });
      }
    });
  ...
  observer.observe(document, {
    ...
    attributeFilter: ['style', 'class', 'src', 'data-meta-tag']
  });
  
  console.log('✅ 画像消失監視を開始しました。10秒後に自動停止します。');
  
  setTimeout(() => {
    observer.disconnect();
    console.log('⏹️ 画像消失監視を停止しました');
  }, 10000);
}

// グローバルに公開
window.watchImageDisappearance = watchImageDisappearance;
