...
function getEnglishTextFromSlotPool(subslotId, parentSlotId) {
  // 🔍 デバッグ：M1、S、M2、O1サブスロット問題調査のため一時的にDOMから直接取得
  if (subslotId.includes('slot-m1-sub-') || subslotId.includes('slot-s-sub-') || subslotId.includes('slot-m2-sub-') || subslotId.includes('slot-o1-sub-')) {
    console.log(`🔍 M1、S、M2、またはO1サブスロットのため、DOM直接取得でテスト: ${subslotId}`);
...
function updateSubslotImages(parentSlotId) {
  ...
  // 🎯 水平展開段階：C1、M1、S、M2、およびO1スロットに対応
  if (parentSlotId !== 'c1' && parentSlotId !== 'm1' && parentSlotId !== 's' && parentSlotId !== 'm2' && parentSlotId !== 'o1') {
    console.log(`⏭️ 水平展開段階のため ${parentSlotId} はスキップします（C1、M1、S、M2、O1スロットのみ対象）`);
    return;
  }
